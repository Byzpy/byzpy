...
def test_select_adaptive_chunk_size_no_pool():
    assert (
        select_adaptive_chunk_size(
            total_items=1000, configured_chunk=256, pool_size=None
        )
        == 256
    )
...
def test_select_adaptive_chunk_size_caps_to_total():
    assert (
        select_adaptive_chunk_size(total_items=10, configured_chunk=256, pool_size=None)
        == 10
    )
...
def test_select_adaptive_chunk_size_env_min_chunks(monkeypatch):
    ...
    chunk = select_adaptive_chunk_size(
        total_items=1048576, configured_chunk=65536, pool_size=4
    )
    assert chunk < 65536
...
def test_select_adaptive_chunk_size_env_target_factor(monkeypatch):
    ...
    chunk = select_adaptive_chunk_size(
        total_items=131072, configured_chunk=32768, pool_size=2
    )
    assert chunk < 32768
