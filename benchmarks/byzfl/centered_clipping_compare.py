#!/usr/bin/env python
"""
Benchmark the ByzFL implementation of Centered Clipping.
"""
...
def main() -> None:
    ...
    def _factory() -> object:
        return byzfl_cls(m=None, L=args.iters, tau=args.tau)
    ...
    try:
        ...
        runs = [BenchmarkRun("ByzFL Centered Clipping", avg_seconds)]
    except TimeoutError:
        label = "ByzFL Centered Clipping (timeout)"
        placeholder = limit if limit is not None else float("inf")
        runs = [BenchmarkRun(label, placeholder)]

    print("\nCentered Clipping (ByzFL)")
    print("-------------------------")
    for run in runs:
