#!/usr/bin/env python
"""
Benchmark the ByzFL implementation of Smallest Maximum Eigenvalue Averaging (SMEA).
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark ByzFL's SMEA implementation."
    )
    parser.add_argument(
...
def _require_byzfl() -> "type[object]":
    try:
        from byzfl.aggregators.aggregators import SMEA as ByzflSMEA
...
def main() -> None:
    ...
    print("\nSmallest Maximum Eigenvalue Averaging (ByzFL)")
    print("---------------------------------------------")
    for run in runs:
