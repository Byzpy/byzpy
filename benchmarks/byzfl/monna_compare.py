#!/usr/bin/env python
"""Benchmark the ByzFL implementation of MoNNA."""
...
def _require_byzfl() -> "type[object]":
    try:
        from byzfl.aggregators.aggregators import MoNNA as ByzflMoNNA
...
def main() -> None:
    ...
    aggregator = byzfl_cls(f=args.f, idx=args.reference_index)
    ...
    try:
        ...
        runs = [BenchmarkRun("ByzFL MoNNA", avg_seconds)]
    except TimeoutError:
        label = "ByzFL MoNNA (timeout)"
