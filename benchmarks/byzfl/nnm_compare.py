#!/usr/bin/env python
"""
Benchmark the ByzFL implementation of Nearest Neighbor Mixing (NNM).
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark ByzFL's NNM pre-aggregator."
    )
    parser.add_argument(
        "--num-vectors", type=int, default=196, help="Number of vectors (n)."
    )
    parser.add_argument("--dim", type=int, default=4096, help="Vector dimension.")
    parser.add_argument("--f", type=int, default=32, help="Expected Byzantine count.")
...
def _require_byzfl() -> "type[object]":
    try:
        from byzfl.aggregators.preaggregators import NNM as ByzflNNM
...
def main() -> None:
    ...
    try:
        ...
        runs = [BenchmarkRun("ByzFL NNM", avg_seconds)]
    except TimeoutError:
        label = "ByzFL NNM (timeout)"
        placeholder = limit if limit is not None else float("inf")
        runs = [BenchmarkRun(label, placeholder)]

    print("\nNearest Neighbor Mixing (ByzFL)")
    print("--------------------------------")
    for run in runs:
