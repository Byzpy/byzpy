#!/usr/bin/env python
"""
Benchmark the ByzFL implementation of Coordinate-wise Trimmed Mean (TrMean).
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark ByzFL's Coordinate-wise Trimmed Mean."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--f", type=int, default=8, help="Trim parameter (drop f values per side)."
    )
...
def _require_byzfl() -> "type[object]":
    try:
        from byzfl.aggregators.aggregators import TrMean as ByzflTrMean
...
def main() -> None:
    ...
    try:
        ...
        runs = [BenchmarkRun("ByzFL TrMean", avg_seconds)]
    except TimeoutError:
        label = "ByzFL TrMean (timeout)"
        placeholder = limit if limit is not None else float("inf")
        runs = [BenchmarkRun(label, placeholder)]

    print("\nCoordinate-wise Trimmed Mean (ByzFL)")
    print("------------------------------------")
    for run in runs:
