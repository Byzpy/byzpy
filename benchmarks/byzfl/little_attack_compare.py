#!/usr/bin/env python
"""
Benchmark the ByzFL implementation of the A Little Is Enough (ALIE) attack.
"""
...
def _require_byzfl() -> "type[object]":
    try:
        from byzfl.attacks.attacks import ALittleIsEnough as ByzflALIE
...
def main() -> None:
    ...
    try:
        ...
        runs = [BenchmarkRun("ByzFL ALIE attack", avg_seconds)]
    except TimeoutError:
        label = "ByzFL ALIE attack (timeout)"
        placeholder = limit if limit is not None else float("inf")
        runs = [BenchmarkRun(label, placeholder)]

    print("\nA Little Is Enough (ByzFL)")
    print("--------------------------")
    for run in runs:
