#!/usr/bin/env python
"""
Benchmark the ByzFL implementation of Mean-of-Medians (Meamed).
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark ByzFL's Meamed aggregator.")
    parser.add_argument(
    ...
    parser.add_argument("--f", type=int, default=8, help="Trim parameter (drop f).")
...
def _require_byzfl() -> "type[object]":
    try:
        from byzfl.aggregators.aggregators import Meamed as ByzflMeamed
...
def main() -> None:
    ...
    try:
        ...
        runs = [BenchmarkRun("ByzFL Meamed", avg_seconds)]
    except TimeoutError:
        label = "ByzFL Meamed (timeout)"
        placeholder = limit if limit is not None else float("inf")
        runs = [BenchmarkRun(label, placeholder)]

    print("\nMean-of-Medians (ByzFL)")
    print("-----------------------")
    for run in runs:
