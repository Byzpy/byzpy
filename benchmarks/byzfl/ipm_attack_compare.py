#!/usr/bin/env python
"""
Benchmark the ByzFL Inner Product Manipulation (IPM) attack.
"""
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--num-grads", type=int, default=64, help="Number of honest gradients."
    )
    parser.add_argument(
        "--grad-dim", type=int, default=65536, help="Gradient dimension."
    )
    parser.add_argument(
        "--tau", type=float, default=2.0, help="Attack scale factor (tau)."
    )
...
def _require_byzfl() -> "type[object]":
    try:
        from byzfl.attacks.attacks import InnerProductManipulation as ByzflIPM
...
def main() -> None:
    ...
    grads = _make_grads(args.num_grads, args.grad_dim, args.seed, device)
    byzfl_cls = _require_byzfl()
    aggregator = byzfl_cls(tau=args.tau)
    ...
    try:
        ...
        runs = [BenchmarkRun("ByzFL IPM attack", avg_seconds)]
    except TimeoutError:
        label = "ByzFL IPM attack (timeout)"
        placeholder = limit if limit is not None else float("inf")
        runs = [BenchmarkRun(label, placeholder)]

    print("\nInner Product Manipulation (ByzFL)")
    print("----------------------------------")
    for run in runs:
