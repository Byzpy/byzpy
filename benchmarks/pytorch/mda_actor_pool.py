#!/usr/bin/env python
"""
Benchmark the Minimum Diameter Averaging (MDA) aggregator with and without
distributed execution through the ActorPool. MDA performs a combinatorial search
over subsets of gradients, which quickly becomes CPU-bound in Python; chunking
the subset search across worker actors dramatically reduces wall-clock time.
"""
...
def _time_direct(
    aggregator: MinimumDiameterAveraging,
    grads: Sequence[torch.Tensor],
    *,
    iterations: int,
    warmup: int,
...
async def _benchmark(args: argparse.Namespace) -> list[BenchmarkRun]:
    ...
    direct_aggregator = MinimumDiameterAveraging(f=args.f, chunk_size=args.chunk_size)
