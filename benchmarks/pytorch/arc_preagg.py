#!/usr/bin/env python
"""Benchmark the Adaptive Robust Clipping (ARC) pre-aggregator."""
...
def _time_direct(
    pre: ARC, vecs: Sequence[torch.Tensor], *, iterations: int, warmup: int
) -> float:
...
async def _benchmark(args: argparse.Namespace) -> list[BenchmarkRun]:
    ...
    graph = make_single_operator_graph(
        node_name="arc",
        operator=ARC(f=args.f, chunk_size=args.chunk_size),
        input_keys=("vectors",),
    )
...
def _print_results(runs: Sequence[BenchmarkRun]) -> None:
    ...
    print("\nAdaptive Robust Clipping Pre-Aggregator Benchmark")
    print("-------------------------------------------------")
