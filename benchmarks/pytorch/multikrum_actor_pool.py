#!/usr/bin/env python
"""
Benchmark the Multi-Krum aggregator with and without ActorPool execution.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark Multi-Krum with ActorPool vs PyTorch."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--f", type=int, default=8, help="Number of Byzantine gradients."
    )
    parser.add_argument(
        "--q", type=int, default=8, help="Number of gradients to average."
    )
    parser.add_argument(
...
async def _benchmark(args: argparse.Namespace) -> list[BenchmarkRun]:
    ...
    graph = make_single_operator_graph(
        node_name="multikrum",
        operator=MultiKrum(f=args.f, q=args.q, chunk_size=args.chunk_size),
