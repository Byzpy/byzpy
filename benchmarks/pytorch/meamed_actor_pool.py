#!/usr/bin/env python
"""
Benchmark the Coordinate-wise Mean of Medians aggregator.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark MeanOfMedians with ActorPool vs PyTorch."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--f", type=int, default=8, help="Trim parameter (drop f farthest)."
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
...
async def _benchmark(args: argparse.Namespace) -> list[BenchmarkRun]:
    ...
    aggregator = MeanOfMedians(f=args.f, chunk_size=args.chunk_size)
    ...
    graph = make_single_operator_graph(
        node_name="meamed",
        operator=MeanOfMedians(f=args.f, chunk_size=args.chunk_size),
