#!/usr/bin/env python
"""
Benchmark the Empire attack with and without ActorPool chunking.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark the Empire attack.")
    parser.add_argument(
        "--num-grads", type=int, default=128, help="Number of honest gradients."
    )
    parser.add_argument(
        "--grad-dim", type=int, default=131072, help="Gradient dimension."
    )
    parser.add_argument(
        "--chunk-size", type=int, default=32, help="Gradients per subtask."
    )
    parser.add_argument(
        "--scale", type=float, default=-1.0, help="Empire scaling factor."
    )
...
def _make_honest_grads(n: int, dim: int, seed: int) -> list[torch.Tensor]:
...
def _time_direct(
    attack: EmpireAttack, grads: Sequence[torch.Tensor], *, iterations: int, warmup: int
) -> float:
...
async def _time_scheduler(
    ...
    *,
    ...
    async def _run_once():
        await scheduler.run({"honest_grads": grads})
...
async def _benchmark(args: argparse.Namespace) -> list[BenchmarkRun]:
    ...
    grads = _make_honest_grads(args.num_grads, args.grad_dim, args.seed)
    attack = EmpireAttack(scale=args.scale, chunk_size=args.chunk_size)

    direct = _time_direct(attack, grads, iterations=args.repeat, warmup=args.warmup)

    graph = make_single_operator_graph(
        node_name="empire",
        operator=EmpireAttack(scale=args.scale, chunk_size=args.chunk_size),
