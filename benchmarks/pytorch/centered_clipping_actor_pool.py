#!/usr/bin/env python
"""
Benchmark the Centered Clipping aggregator with and without ActorPool execution.
"""
...
import torch
from byzpy.aggregators.norm_wise.center_clipping import CenteredClipping
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark Centered Clipping with ActorPool vs PyTorch."
    )
    parser.add_argument(
    ...
    parser.add_argument("--c-tau", type=float, default=0.1, help="Clipping radius.")
    parser.add_argument(
        "--iters",
        type=int,
        default=10,
        help="Number of centered clipping iterations (M).",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=16,
        help="Number of gradients processed per subtask.",
    )
...
async def _benchmark(args: argparse.Namespace) -> list[BenchmarkRun]:
    ...
    aggregator = CenteredClipping(
        c_tau=args.c_tau, M=args.iters, chunk_size=args.chunk_size
    )
    ...
    graph = make_single_operator_graph(
        node_name="centered_clipping",
        operator=CenteredClipping(
            c_tau=args.c_tau, M=args.iters, chunk_size=args.chunk_size
        ),
