#!/usr/bin/env python
"""
Benchmark the Coordinate-wise Trimmed Mean aggregator with and without ActorPool execution.
"""
...
async def _benchmark(args: argparse.Namespace) -> list[BenchmarkRun]:
    ...
    graph = make_single_operator_graph(
        node_name="cwtm",
        operator=CoordinateWiseTrimmedMean(f=args.f, chunk_size=args.chunk_size),
...
def _print_results(runs: Sequence[BenchmarkRun]) -> None:
    ...
    print("\nCoordinate-wise Trimmed Mean Benchmark")
    print("--------------------------------------")
