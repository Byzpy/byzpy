#!/usr/bin/env python
"""
Benchmark the Nearest Neighbor Mixing pre-aggregator.
"""
...
async def _benchmark(args: argparse.Namespace) -> list[BenchmarkRun]:
    ...
    graph = make_single_operator_graph(
        node_name="nnm",
        operator=NearestNeighborMixing(f=args.f, feature_chunk_size=args.chunk_size),
        input_keys=("vectors",),
    )
...
def _print(runs: Sequence[BenchmarkRun]) -> None:
    ...
    print("\nNearest Neighbor Mixing Benchmark")
    print("---------------------------------")
