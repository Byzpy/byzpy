#!/usr/bin/env python
"""
Benchmark the Smallest Maximum Eigenvalue Averaging (SMEA) aggregator using the simplified API.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark SMEA using simplified API.")
    parser.add_argument(
        "--num-grads", type=int, default=12, help="Number of gradients (n)."
    )
    parser.add_argument(
        "--grad-dim", type=int, default=1024, help="Gradient dimension."
    )
    parser.add_argument(
...
def _print_results(runs: Sequence[BenchmarkRun]) -> None:
    ...
    print("\nSmallest Maximum Eigenvalue Averaging Benchmark (Simplified API)")
    print("---------------------------------------------------------------")
